...
def staircase(k=5):
    """Worst-Case-Treppe: die Ketten 1..k (Kette i = `long_chain(i)`, i + 1 Fahrzeuge und Aufträge) liegen in eigenen Zeilen (Abstand 14, außer Reichweite
    füreinander). Fahrzeuge stehen je Kette absteigend, Aufträge aufsteigend im Index: dann wählt sowohl Greedy als auch das Erste-passende-Verfahren
    des leeren Starts alle billigen Kanten (Kosten 2). Jede Kette hat genau einen Verbesserungsweg der Länge 2i + 1, also braucht Hopcroft-Karp genau k
    Phasen (Phase t klappt Kette t um); n = m = k (k + 3) / 2. Für k > 7 reicht die Karte über 100 hinaus (nur für das Aufwand-Experiment)."""
    vehicles, orders = [], []
    for i in range(1, k + 1):
        y = 5 + 14 * (i - 1)
        vehicles += [(12 * j, y) for j in range(i, -1, -1)]
        orders += [(12 * j + 10, y) for j in range(i + 1)]
    return from_points(vehicles, orders, reach=10)
